...
(`.github/workflows/publish.yml`, at the repository root) builds the image for
amd64 and arm64 and publishes it to GitHub Container Registry whenever anything
under `DB-PT/` changes. Nothing to configure.

After the first run, make the package public: **your profile → Packages →
pt/db-pt → Package settings → Change visibility → Public**. If you'd
...
| Repository URL | `https://github.com/ArztBlut/PT` |
| Repository reference | `refs/heads/main` |
| Compose path | `DB-PT/compose.yaml` for PostgreSQL, `DB-PT/compose.mariadb.yaml` for MariaDB |
...
| `APP_IMAGE` | `ghcr.io/arztblut/pt/db-pt:latest` |
